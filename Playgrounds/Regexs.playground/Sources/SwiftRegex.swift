...
public class SwiftRegex: NSObject { //, ExpressibleByBooleanLiteral {
    ...
    func groupsForMatch( match: NSTextCheckingResult! ) -> [String] {
        if match != nil {
            ...
            for groupno in 0...regex.numberOfCaptureGroups {
                let group = substring(range: match.range(at: groupno))
                groups.append(group)
            }
            return groups
        } else {
    ...
    public subscript( groupno: Int ) -> String {
        get {
            ...
            return group != regexNoGroup ? group : ""
        }
        set(newValue) {
    ...
    public func matches( options: NSRegularExpression.MatchingOptions? = nil ) -> [String] {
        return matchResults( options: options ).compactMap { self.substring(range: $0.range) }
    }
...
public func ~= ( left: String, right: String ) -> Bool {
    let groups = SwiftRegex( target: right as NSString, pattern: left ).groups()
