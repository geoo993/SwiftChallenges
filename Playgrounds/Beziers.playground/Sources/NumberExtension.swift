...
extension CGPoint: NumberFormatProvider {
    public func formatNumber() -> NSNumber {
        return NSNumber()
    }

    public func format(_ formatter: NumberFormatter) -> String {
        let formattedX = NumberFormat.sharedInstance.format(x as NSNumber, formatter: formatter)
        let formattedY = NumberFormat.sharedInstance.format(y as NSNumber, formatter: formatter)
        return "CGPoint(x: \(formattedX), y: \(formattedY))"
    }
}
